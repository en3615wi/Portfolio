package se.lu.ics.controllers;
    ...
    private String generateRandomVIN() {
        String vinLetters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        String vinDigits = "0123456789";
        int vinLetterCount = 3;
        int vinDigitCount = 3;

        StringBuilder vinBuilder = new StringBuilder();

        // Add random letters
        for (int i = 0; i < vinLetterCount; i++) {
            vinBuilder.append(getRandomCharacter(vinLetters));
        }

        // Add random digits
        for (int i = 0; i < vinDigitCount; i++) {
            vinBuilder.append(getRandomCharacter(vinDigits));
        }
